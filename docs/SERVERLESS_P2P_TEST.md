...
- Both instances have NOSTR relays configured. The defaults
  (`relay.damus.io`, `nos.lol`, `relay.nostr.band`, `relay.primal.net`)
  are fine; check `/api/status` if unsure.
- Both instances know each other's npub. The simplest way: pair them
  once via the existing Mirror flow, or just paste each npub into the
  other's contact list before the test. The test asserts on
  `npub`-keyed addressing, not callsign.
...
POST /api/p2p/serverless/signal                → send one WebRTC signal via NOSTR-DM
...
- `dht_running: true`
- `dht_routing_size > 0`
- `reachability.status` is one of `reachableIPv6` / `reachableUPnP` /
...
- `dht_blocked: false` (true means the DHT bootstrap got no responses
  in 30s — fall back to NOSTR presence; see Phase-1 §6.7 in the spec).
...
## Step 3 — Phase-2 acceptance: NOSTR-DM signaling round trip

Send a synthetic offer from the desktop directly via the NOSTR-DM
signaling channel and confirm the Android receives it. This exercises
NIP-44 v2 encryption + relay subscribe end-to-end without driving the
full WebRTC stack.
...
- Response: `success: true`, `relays_sent >= 1` (any of the 3+
  configured relays accepted the EVENT frame).
- Desktop log: `NostrSignaling: sent offer to npub1xxxx… via N relay(s)`.
- Android log within ~2s: `NostrSignaling: started for X1XXXX (N relays)`
  (only on first start) and a subsequent
  `WebRTCSignaling: Received offer from <DESKTOP_CALL> via NostrDM
  (session: <SESSION_ID>)`.

If the Android log doesn't show the receive line, common causes:

1. The Android's NOSTR relays are different from the desktop's — set
   them to overlap by at least one (Settings → NOSTR feed).
2. The Android's `NostrSignaling: started` log never fired — check that
   `enableServerless` and `nostrSignalingEnabled` are both `true` in
   `/api/p2p/serverless/status`.
3. The npub addressing is wrong — re-run `/api/p2p/serverless/dht/topic`
   from step 1 to confirm both ends see the same hash.
...
and try to establish a session. The signaling will go via the NOSTR-DM
path.
...
- `WebRTCSignaling: Sent offer to <peer> via NOSTR DM (N relays)` —
  desktop sender
- `WebRTCSignaling: Received answer from <peer> via NostrDM (session: …)`
  — desktop sender

...
static const char *LANDING_PAGE_HTML_SUFFIX =
    ...
    "(async()=>{try{await initKeys();updateStatus();renderStoredMessages();renderStoredAprs();updateCallsignList();await reportClient(clientKeys);initWebSocket();await load();"
    "updateCallsignList();$('messages').scrollTop=$('messages').scrollHeight;"
    "probeAprs();"
    "setInterval(()=>{load().finally(()=>{loadAprs();updateCallsignList();});},3000);}catch(e){"

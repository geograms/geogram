...
class _DeviceSyncPageState extends State<DeviceSyncPage> {
  ...
  Future<void> _loadDiffs() async {
    ...
    // One auth handshake for the whole sync session (instead of one per
    // folder). The session token is valid for every folder so the subsequent
    // transfer phase can reuse it without a second handshake.
    final session = await mirror.requestSync(
      _peerUrl!,
      MirrorSyncService.sessionFolder,
    );
    if (!session.allowed || session.token == null) {
      LogService().log(
        'DeviceSync: Session auth failed: ${session.error}',
      );
      fileIndex.close();
      if (mounted) {
        setState(() {
          _loadingDiffs = false;
          _diffError =
              'Could not authenticate with peer (${session.error ?? 'unknown'}). '
              'Check that mirror sync is enabled on the peer device.';
        });
      }
      return;
    }
    ...
    for (final folder in _appFolders) {
      _tokens[folder] = sessionToken;
    }

    // One HTTP call fetches every folder's manifest.
    final allManifests = await mirror.fetchAllManifests(_peerUrl!, sessionToken);
    if (allManifests == null) {
      LogService().log('DeviceSync: manifest_all fetch failed');
      fileIndex.close();
      if (mounted) {
        setState(() {
          _loadingDiffs = false;
          _diffError =
              'Could not fetch file lists from peer. '
              'Check that mirror sync is enabled on the peer device.';
        });
      }
      return;
    ...
    // Local diff per folder in parallel — no network from here on.
    Future<void> diffOne(String folder) async {
      try {
        final manifest = allManifests[folder];
        ...
        LogService().log('DeviceSync: Error diffing $folder: $e');
        foldersErrored++;
      } finally {
        done++;
        if (mounted) {
    ...
    for (var i = 0; i < _appFolders.length; i += _kFolderConcurrency) {
      final end = (i + _kFolderConcurrency < _appFolders.length)
          ...
          : _appFolders.length;
      await Future.wait(_appFolders.sublist(i, end).map(diffOne));
    }
    ...
    if (mounted) {
      setState(() {
        ...
        _currentDiffFolder = null;
        if (_diffs.isEmpty) {
          if (foldersErrored > 0 && foldersCompared == 0) {
            _diffError =
                'Could not compare any apps ($foldersErrored errored). '
  ...
  Future<void> _loadMultiDiffs() async {
    ...
    // across every selected device.
    final pushEntriesPerFolder = <String, Map<String, MirrorFileEntry>>{
      for (final f in _appFolders) f: <String, MirrorFileEntry>{},
    };

    // One auth + one manifest_all per device (was: 3 × folders × devices).
    // Cache manifests by peerUrl so devices that share a peer only fetch once.
    final manifestsByPeer = <String, Map<String, MirrorManifest>>{};
    for (final entry in _multiPeerUrls.entries) {
      final deviceId = entry.key;
      final peerUrl = entry.value;
      try {
        final session = await mirrorService.requestSync(
          peerUrl,
          MirrorSyncService.sessionFolder,
        );
        if (!session.allowed || session.token == null) {
          LogService().log(
            'DeviceSync: Session auth failed for $deviceId: ${session.error}',
          );
          continue;
        }
        final sessionToken = session.token!;
        _multiTokens.putIfAbsent(deviceId, () => {});
        for (final folder in _appFolders) {
          _multiTokens[deviceId]![folder] = sessionToken;
        }
        manifestsByPeer[peerUrl] ??=
            (await mirrorService.fetchAllManifests(peerUrl, sessionToken)) ??
                const {};
      } catch (e) {
        LogService().log(
          'DeviceSync: Multi-diff setup error for $deviceId: $e',
        );
      }
    }

    // Local diff per (folder, device) tuple. No network from here on.
    ...
    Future<void> diffOne(({String folder, String deviceId, String peerUrl}) w) async {
      try {
        final peerManifests = manifestsByPeer[w.peerUrl];
        if (peerManifests == null) return;
        final manifest = peerManifests[w.folder];
        if (manifest == null) return;
        foldersCompared++;
        ...
        workDone++;
        if (mounted && workDone % _multiPeerUrls.length == 0) {
          setState(() {
            _currentDiffFolder = kFolderLabels[w.folder]?.$1 ?? w.folder;
            _diffFoldersDone = workDone ~/ _multiPeerUrls.length;
          });
        }
      }
    }
    ...
    for (var i = 0; i < work.length; i += _kFolderConcurrency) {
      final end = (i + _kFolderConcurrency < work.length)
          ? i + _kFolderConcurrency
          : work.length;
      await Future.wait(work.sublist(i, end).map(diffOne));
    }

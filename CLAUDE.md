...
- DO NOT DUPLICATE CODE FUNCTIONALITY. Write libraries and make sure that apps are mostly using a single file without duplication of functionality in other parts of the code

- When writing code for a station server, there are two implementations so write code that uses a library or common method that is called by the two implementations.
